...
# Write file
# write_csv(Flow.subset.clean, "data_export/flow_subsets_for_alts_equalspaced.csv")
write_csv(Flow.subset.summary, "data_export/flow_subsets_samplesize_for_alts_equalspaced.csv")
...
(plot_allvals <- ggplot() +
    ...
    geom_point(data = Flow.subset.clean %>%filter(OMR>-6500), aes(x = Sac.cfs, y = SJR.cfs, color = Sub.group, shape = factor(OMR)), size = 4) +
...
# Write plot
png("figures/allflowvals_groups_equalspaced_plot.png", width = 8, height = 5, units = "in", res = 300, pointsize = 12)
plot_allvals
dev.off()
